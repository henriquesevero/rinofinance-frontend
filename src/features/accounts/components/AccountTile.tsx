...
export function AccountTile({ account, className }: { account: Account; className?: string }) {
  ...
  return (
    <Link to={`/accounts/${account.id}`} viewTransition className="group block">
      <div
        className={cn(
          "@container relative flex aspect-[1.3] flex-col justify-between overflow-hidden rounded-2xl p-3 shadow-sm ring-1 ring-black/10 transition-all duration-300 ease-out group-hover:-translate-y-1 group-hover:shadow-[0_16px_44px_-16px_rgba(97,218,251,0.5)]",
          className
        )}
      ...
      >
        ...
        <div className="absolute inset-0 flex items-center justify-center md:hidden">
          <AccountAvatar account={account} className="size-24 rounded-xl ring-0" />
        </div>
      </div>
    </Link>
  )
