...
export function InstallmentPurchaseFormDialog({
  ...
  useEffect(() => {
    if (open) {
      ...
      setInstallmentAmount(purchase?.installmentAmount ?? 0)
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            {oneOff
              ? "Uma compra à vista lançada na fatura do cartão."
              : "Uma compra dividida em parcelas mensais iguais."}
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <Input
              id="purchase-name"
              placeholder="Ex: iPhone 16, Mercado, Farmácia..."
              required
              value={name}
          ...
          <div className={cn("grid gap-4", !oneOff && "sm:grid-cols-2")}>
            {!oneOff && (
              <div className="flex flex-col gap-1.5">
            ...
            <div className="flex flex-col gap-1.5">
              <Label htmlFor="purchase-date">{oneOff ? "Data da compra" : "Data da 1ª parcela"}</Label>
              <Input
                id="purchase-date"
                type="date"
                required
